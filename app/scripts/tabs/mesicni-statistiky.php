...
if(MySQLi_num_rows($qStat) > 2)
{
    ...
    if(kolikRadek("den", "tme_denni", "GROUP BY year(den), month(den)") > 2)
    {
        echo "<div class='graf' id='graf-mesicni-teplota'>";
        require dirname(__FILE__) . '/../grafy/teplota/mesicni.php';
        echo "</div>";
    }
    ...
    if($vlhkomer == 1)
    {
        if(kolikRadek("den", "tme_denni", "GROUP BY year(den), month(den)") > 2)
        {
            echo "<div class='graf' id='graf-mesicni-vlhkost'>";
            require dirname(__FILE__) . '/../grafy/vlhkost/mesicni.php';
            echo "</div>";
        }
    }
